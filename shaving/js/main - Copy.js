...
function refreshReviews(tab) {
	...
	//if (tab == undefined || tab == null)
		//tabsObj.NestedTabs[1].ToggleTab(oldSelectedTab); // toggle previously selected
	//else
...
function createJournalSorter(sort_field, sort_dir, filter) {
	//console.log("createJournalSorter(sort_field, sort_dir, filter)");
	var sorter = document.getElementById('journal_sorter');
	sorter.innerHTML = "";
	//DATE
	var sortField = document.createElement('span');
	sortField.appendChild(document.createTextNode('Date'));
	var sort_img = document.createElement('img');
	sort_img.setAttribute('class', "sort_arrow");
	var filter_txt = ((filter == null) ? "null" : "'"+filter.replace(/'/g, "\\\'")+"'");
	if (sort_field == 0 && sort_dir == "ASC") {
		sort_img.setAttribute('src', "img/sort_up.png");
		sortField.appendChild(sort_img);
		sortField.setAttribute('title', "Sort Descending");
		sortField.setAttribute('onclick', "loadShavingJournal(0, 'DESC', "+filter_txt+");");
	} else {
		if (sort_field == 0) {
		...
		sortField.setAttribute('onclick', "loadShavingJournal(0, 'ASC', "+filter_txt+");");
	}
	sorter.appendChild(sortField);
	//GRADE
	sortField = document.createElement('span');
	sortField.appendChild(document.createTextNode('Grade'));
	sort_img = document.createElement('img');
	sort_img.setAttribute('class', "sort_arrow");
	if (sort_field == 1 && sort_dir == "ASC") {
		sort_img.setAttribute('src', "img/sort_up.png");
		sortField.appendChild(sort_img);
		sortField.setAttribute('title', "Sort Descending");
		sortField.setAttribute('onclick', "loadShavingJournal(1, 'DESC', "+filter_txt+");");
	} else {
		if (sort_field == 1) {
			sort_img.setAttribute('src', "img/sort_down.png");
			sortField.appendChild(sort_img);
		...
		sortField.setAttribute('title', "Sort Ascending");
		sortField.setAttribute('onclick', "loadShavingJournal(1, 'ASC', "+filter_txt+");");
	}
	sorter.appendChild(sortField);
}
//var row_count = 0;
function loadShavingJournal(sort_field, sort_dir, filter) {
	...
	createJournalSorter(sort_field, sort_dir, filter);

	var entries = ShavingJournalEntries.slice();
	...
	for (var ii = num_entries; ii > -1; ii--) {
		var i = (sort_dir == "DESC") ? ii : (num_entries-ii);
		// console.log("ENTRY #"+i+": ", entries[i]);
		//console.log("ENTRY #"+0+": ", entries[0]);
		if (filter != null && !eval(filter)) continue;
		...
		//setStyle(entry, "background-image: url(\"img/shave_grade_meh.png\");");
		setStyle(entry, "background-image: url(\"img/shave_grade_"+entries[i].Grade.toLowerCase()+".png\");");
		//if (!(i % 2)) entry.setAttribute("class", "distinct_row");
		if (!(displayed_entries % 2)) entry.setAttribute("class", "distinct_row");
		...
		for (var cat_row = 0; cat_row < categories.length; cat_row++) {
			...
			for (var cat = 0; cat < categories[cat_row][0].length; cat++) {
				...
				var cat_val = entries[i][cat_name];
				...
				if (cat_name == "Grades") {
					...
					if (cat_name == "Number of Passes") {
						...
						if (hasReview) {
							...
							var rev_idx = ShavingReviewedProducts.indexOf(entries[i][cat_name.replace(/\//, "")]);
		...
		note.innerHTML = "<hr />"+entries[i].Notes;
		//note.appendChild(document.createTextNode(entries[i].Notes));


        var sotd = null;
        if (ShavingAlbum != null) {
            sotd = ShavingAlbum.getPicture("SOTD_"+
					HG_formatDate(entries[i].Date, "%Y%M%D")+".jpg");
        }
		...
		if (entry != undefined) delete entry;
...
function loadForms() {
	...
	var spans = document.getElementById('forms').getElementsByTagName('div');
	// Grab all spans
	...
	loadReviewGrades("Razor", "review_grade_scales");
	var prodIDField = document.getElementById("review_entry_id");
...
function loadExtras() {
    ...
    HG_loadJournalViews([new HG_Journals_View("ShavingJournalEntries", 
                         ...
                         new HG_Journals_View("ShavingReviewedProducts", 
                                              document.getElementById("review_tabs"), 
                                              function() {
